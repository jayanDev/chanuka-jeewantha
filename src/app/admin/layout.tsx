...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <section className="w-full min-h-screen bg-zinc-50 dark:bg-zinc-900 py-10">
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 space-y-6">
        <header className="rounded-[20px] border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 md:p-8">
        ...
        <nav className="rounded-[16px] border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-3">
          <ul className="grid grid-cols-1 gap-2 md:grid-cols-5">
            {adminLinks.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="block rounded-[10px] border border-zinc-200 dark:border-zinc-800 px-4 py-3 text-sm font-medium text-zinc-700 dark:text-zinc-300 transition hover:border-brand-main hover:text-brand-main"
                >
                  {item.label}
                </Link>
              </li>
