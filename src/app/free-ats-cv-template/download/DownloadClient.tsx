...
const DOWNLOAD_HREF = "/api/free-cv-template/download";
const FILE_NAME = "ATS Friendly Professional CV Format - Chanuka Jeewantha.docx";

type DownloadState = "idle" | "downloading" | "done" | "error";

const tips = [
  "Replace the sample text with your own details — keep the section order.",
  "Use clear job titles and 2–4 bullet points of measurable achievements per role.",
  "Save your final CV as PDF when applying, unless the employer asks for Word.",
];
    ...
    <div className="rounded-[16px] border-2 border-brand-main/40 bg-white p-6 shadow-lg sm:p-8">
      <div className="flex flex-col items-center text-center">
        ...
        <h2 className="mt-5 font-heading text-[26px] font-bold text-foreground sm:text-[30px]">
          {state === "done" ? "Your download has started 🎉" : "Your free CV format is ready"}
        </h2>
        <p className="mt-2 max-w-md text-sm leading-relaxed text-text-body">
          {state === "done"
            ...
            : "Microsoft Word format (.docx) · Fully editable · No signup required."}
        </p>
      ...
      <p className="mt-6 text-center text-sm text-text-secondary">
        Looking for more?{" "}
        <Link href="/resources" className="font-semibold text-brand-dark hover:text-brand-main">
          Browse free career resources
        </Link>
      </p>
    </div>
  );
